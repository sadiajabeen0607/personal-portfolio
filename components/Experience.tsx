...
const ExperienceItem = ({
  ...
  const x = useTransform(scrollYProgress, [start, end], [-12, 0]);

  if (layout === "desktop") {
    return (
      <div className="relative flex flex-1 justify-center items-center min-w-0">
        <motion.div
          className="z-10 w-7 h-7 rounded-full bg-[#6366f1] shadow-[0_0_0_8px_rgba(255,255,255,0.1)]"
          style={{ scale, opacity }}
        />
        ...
        <motion.article
          ...
          } bg-bg/90 backdrop-blur border border-[#6366f1]/70 rounded-xl p-7 w-80 shadow-lg`}
          style={{ opacity, y, maxWidth: "90vw" }}
        >
          <h3 className="text-xl font-semibold text-[#6366f1]">{exp.role}</h3>
          <p className="text-md mb-3 text-(--muted)">
  ...
  return (
    <div className="relative flex items-start">
      <motion.div
        className="absolute -left-3.5 top-3 z-10 w-7 h-7 rounded-full bg-[#6366f1] shadow-[0_0_0_8px_rgba(255,255,255,0.1)]"
        style={{ scale, opacity }}
      />

      <motion.article
        className="glass backdrop-blur border border-text/70 rounded-xl p-5 w-[90vw] max-w-sm ml-6 shadow-lg"
        style={{ opacity, x }}
      >
...
export default function Experience() {
  ...
  const SCENE_HEIGHT_VH = isMobile
    ? 110 * experiences.length
    : 90 * experiences.length;
  ...
  const thresholds = useMemo(
    () => experiences.map((_, i) => i / (experiences.length - 1)),
    [],
  );
    ...
    <section id="experience" className="relative bg-bg text-text min-h-screen">
      ...
      >
        <div className="sticky top-0 h-screen flex flex-col">
          <h2 className="text-4xl sm:text-5xl font-semibold mt-5 text-center text-transparent bg-clip-text bg-linear-to-r from-[#7bdfee] to-[#15177c] drop-shadow-lg">
            Experience
          </h2>
            ...
            {!isMobile && (
              <div className="relative w-full max-w-7xl">
                <div className="relative h-1.5 bg-[#6366f1]/15 rounded">
                  <motion.div
                    className="absolute left-0 top-0 h-1.5 bg-[#6366f1] rounded origin-left"
                  ...
                  {experiences.map((exp, idx) => {
                    ...
                    const start =
                      idx === experiences.length - 1
                        ? Math.max(0, end - ITEM_WINDOW * 1.4)
                        : Math.max(0, end - ITEM_WINDOW);
            ...
            {isMobile && (
              <div className="relative w-full max-w-md">
                <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-[#6366f1] rounded">
                  <motion.div
                    className="absolute top-0 left-0 w-1.5 bg-[#6366f1] rounded origin-top"
